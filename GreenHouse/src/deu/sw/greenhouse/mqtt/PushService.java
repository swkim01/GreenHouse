...
public class PushService extends Service
{
	...
	public void requestGraphData(String graphID)
	{
		...
		{
			if(mCallback != null)
				mCallback.sendData(this.timeValueHumidity, "humi");
		}
		else if(graphID.equals("illu"))
		{
			if(mCallback != null)
				mCallback.sendData(this.timeValueIlluminance, "illu");			
		}
	...
	{
		return this.msg_temp;
	}
	
//	public ArrayList<TimeValue> getValueArray()
//	{
...
//	
	...
	private static ArrayList<TimeValue> timeValueTemperature = new ArrayList<TimeValue>();
	// this is the log tag
	...
	public class MQTTConnection implements MqttSimpleCallback {
	    	...
	    	String mqttConnSpec = "tcp://" + brokerHostName + "@" + port;
	        	// Create the client and connect
					...
					try {
						...
						Toast toast = Toast.makeText(getApplicationContext(), "서버에 연결 되었습니다.", Toast.LENGTH_LONG);
		...
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained) throws JSONException {
			...
			///////////////
			if(topicName.equals("greenhouse/sensor/R1/humi")){
				jsonSub(s, "humi");
			}
			else if(topicName.equals("greenhouse/sensor/R1/illu")){
				jsonSub(s, "illu");
			}			
		...
		public void jsonSub(String s, String graphID)
		{
			JSONObject jsonObj;
			try {
...
//				//log("Got message: " + data1);
				...
				if(graphID.equals("humi"))
				{
					timeValueHumidity.clear();
				}
				else if(graphID.equals("illu"))
				{
					timeValueIlluminance.clear();
				}
				else if(graphID.equals("temp"))
				{
					timeValueTemperature.clear();
				}
				...
				for(int i=0; i<ja.length(); i++)
				{
					...
					TimeValue tv = new TimeValue(date, value);
					
					if(graphID.equals("humi"))
					{
						timeValueHumidity.add(tv);
					}
					else if(graphID.equals("illu"))
					{
						timeValueIlluminance.add(tv);
					}
					else if(graphID.equals("temp"))
					{
						timeValueTemperature.add(tv);
					}
				}
		...
		public void MessageToString(String topicName, String message)
		{
			if(topicName.equals("hoyong/sensor/illu"))
			{
				msg_illu = new String(message+"L%");
				log("message_hoyong/sensor/illu: " + msg_illu);
			}
			else if(topicName.equals("hoyong/sensor/humi"))
			{
				msg_humi = new String(message+"%");
				log("message_hoyong/sensor/humi: " + msg_humi);
			}
			else if(topicName.equals("hoyong/sensor/temp"))
			{
