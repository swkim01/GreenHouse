...
//    }
	...
	public void setupGL(GL10 gl)
	{
		try {
			...
			this._mid=this._ms.addARMarker(assetMng.open("AR/data/deusw8.pat"),8,25,80);
			this._ss.start();
			...
			synchronized(this._ss){		
				...
				if(this._ms.isExistMarker(this._mid)){
					...
					for (GraphSquare graph : _graph) {
						graph.setOnClickListener(new GraphClickEventListener() {
...
//		String returnvalue = "";
	...
	{
		if(mBound && s.equals("Humidity"))
		{
			Log.i("test", "request Humidity");
			mService.requestHumidityGraphData();
			mService.requestGraphData("humi");
		}
		else if(mBound && s.equals("Illuminance"))
		{
			Log.i("test", "request Illuminance");
			mService.requestIlluminanceGraphData();
			mService.requestGraphData("illu");
		}
		else if(mBound && s.equals("Temperature"))
		{
			Log.i("test", "request Temperature");
			mService.requestTemperatureGraphData();
